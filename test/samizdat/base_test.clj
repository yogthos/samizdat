  ...
  {"src/samizdat/config.clj"
   ...
   {:threshold {200 "The success range of a status code. Protocol, as in
                ...
                150000 "How long the client waits for POST /v1/runs. Bounded by
                       the SERVER's own start deadline (gates.edn
                       :run-start-deadline-ms, 120 s shipped; control-test pins
                       the ordering) rather than by anything a project chooses — a client bound tighter than
                       the endpoint it calls reports a failure for a run that
                       started normally, which is the bug this number exists to
                       fix. It has to move with that deadline, not with a
                       front end's own table.
                ...
                1500 "The poll interval. See 150000 on why a client's numbers
