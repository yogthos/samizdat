;; Ported to jolt from github.com/yogthos/maestro (src/maestro/core.cljc).
;; Divergences from upstream:
;;   - sci is gone: EDN dispatch predicates compile with the host eval. Safety
;;     against bad agent-authored predicates comes from validate/soak/rollback
;;     at the mutation layer, not from interpreter sandboxing.
;;   - promesa is gone: the async executor drives a loop on its own thread
;;     (jolt future) and parks on a per-step promise, so it is stack-safe for
...
(defn- error-fsm
  "Records a failed step in the trace and routes the machine to ::error."
  [fsm max-trace current-state-id start-time error]
          ...
          (let [start-time (current-time)
                ...
                callback (fn [result-data]
                           (try
                             ...
                             (catch Throwable e
                               (vreset! error-state
                                        (error-fsm fsm max-trace current-state-id start-time e)))))
                error-callback (fn [error]
                                 (vreset! error-state
                                          (error-fsm fsm max-trace current-state-id start-time error)))]
          ...
          (cond
            ...
            (let [start-time (current-time)
                  ...
                  error-callback (fn [error]
                                   (deliver step
                                            (error-fsm fsm max-trace current-state-id start-time error)))]
              ;; A handler that throws before invoking either callback would
              ;; otherwise park this step forever.
              (try
                (handler resources data callback error-callback)
                (catch Throwable e
...
(defn run
  "Executes the FSM spec compiled using compile.
   ...
   For workflows with async handlers, returns a deref-able future."
  ([fsm]
   (run fsm {} {}))
  ([fsm resources]
   (run fsm resources {}))
...
(defn run-async
  "Executes the FSM asynchronously, returns a deref-able future with the
   result regardless of whether the FSM was compiled sync or async."
  ([fsm]
   (run-async fsm {} {}))
  ([fsm resources]
...
(defn- index-of
  "Index of the first occurrence of x in vector v, or -1."
  [v x]
  (loop [i 0]
    (cond
      (>= i (count v)) -1
