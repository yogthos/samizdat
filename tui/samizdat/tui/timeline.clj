...
(defn- turn-entries [t text banner]
  (let [n (:turn t)
        at (str (:created_at t))
        {:keys [assistant_text reasoning_text]} text]
    (cond-> []
      (not (str/blank? (str assistant_text)))
      (conj {:key (str "t" n "/say") :role :agent :kind :say :turn n :at at
             :text (str assistant_text)})

      (not (str/blank? (str reasoning_text)))
...
(defn- note-entries [notes by-kind]
  (for [n notes
        ...
        :when role]
    {:key (str "n" (:id n)) :role role :kind :say :at (str (:created_at n))
     :text (note-text (parse (:data n)) say) :note (str (:kind n))}))
